...
Making use of the amazing background gradients in CSS to create a rich, full-screen banner that loads in less than half a second and weighs less than 2Kb.
...
Here we have an article that contains two spans, each of which has a 7. The reason for including two 7s will become apparent in a moment. Before applying gradients, set up the 7s so that they sit in an iOS-like icon in the middle of the screen.
...
Next, set some attributes for the text.

    .container span.text {
        font-family: "HelveticaNeue-UltraLight", "Helvetica Neue UltraLight", Arial, Sans-serif;
        font-size: 512px;
        position: absolute;
        top: 0;
        left: 124px;
...
The font is "Helvetica Neue UltraLight", and I've added a fallback of Arial. This isn't ideal, so I'll also use the [@font-face](http://docs.webplatform.org/wiki/css/atrules/@font-face) rule to handle fetching an embedded fontin case it's not available in the browser. The text spans are positioned absolutely, so that both 7s will sit directly on top of each other, and both are set to 512 pixels in size.
...
This CSS tells the browser to go fetch the font file if it's not available on the system.
...
TODO: Image of the first 7 with gradient on an icon and black background

The next step is to apply another gradient to the second seven. Since this seven sits in front, it needs to fade to transparent. Its gradient CSS looks like this:
...
<img src="/images/posts/seven/step3.png" alt="The text with gradient applied">
