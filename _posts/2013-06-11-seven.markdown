...
To get around this we can create a [pseudo-element](http://docs.webplatform.org/wiki/css/selectors/pseudo-elements) and apply the pattern to that. pseudo-elements can be added to an element to give us extra "elements" that we can style, referred to as *before* and *after*. Here's the *before* pseudo-element:
...
The *before* element contains the pattern and is itself rotated 45 degrees on the Z axis. At this point it's almost there, it needs the white gradient from the top of the banner. We can use the second pseudo-element to contain that gradient:
