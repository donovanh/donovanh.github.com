...
It is no longer necessary to use JavaScript to create impressive 3D projects in the web browser. Read how to create and animate a Portal-inspired scene using only CSS.
...
This video blew my mind when it was first released. Let's focus on the initial scene, with the isometric, cartoony graphics and see if it can be reproduced using nothing but HTML and CSS.
...
The first property to apply is [perspective](http://docs.webplatform.org/wiki/css/properties/perspective). This property takes a value in pixels, and represents the depth of the 3D scene. A smaller value will produce a more dramatic effect, and it's usual to set it somewhere betweeh 800 and 1200 pixels.
...
TODO http://docs.webplatform.org/wiki/css/properties/transform-origin
...
Two borders are used to create the effect. The bottom border matches the platform colour, making it look like the platform extends into the space in the wall, and the left border matches the border used on the right side of the right wall.
