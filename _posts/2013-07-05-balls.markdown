...
layout: post
title: "CSS balls"
description: "Create great looking spheres using CSS."
tags: [spheres]
imageURL: ball.png
published: true
---
...
There are two ways we could approach making spheres with CSS. One is to create an actual 3D sphere using lots of elements. There are some [beautiful](http://codepen.io/peterwestendorp/pen/wGECk) [examples](http://www.cssplay.co.uk/menu/cssplay-3D-sphere.html) of these. One downside though is that these require the browser display many elements, which can impact performance.

Instead of this I'll make use of the second approach, which is to make use of CSS gradients to add shading and create the 3D effect on a single plane.

## Demo and source code

All the examples mentioned can be found on the [CSS Spheres demo page](/balls/) online, and you can download the [source code on Github](http://github.com/donovanh/balls/).

They are also [on Codepen](http://codepen.io/donovanh).
...
We're using an `i` element here, but it could be any element really. Since it's more common to use `em` now, the `i` tag doesn't get used much, so it's safe enough to use in this instance.

To create a circle from this `i` element, I'll give it a width and height, and a border radius of 100%:
...
Animation can also be applied to the position of background images, such as to create a spinning globe.
...
They can also be found via [my Codepen account](http://codepen.io/donovanh). Many thanks to Chris and the team for making such a fantastic resource.

If you have any questions about the above, I can be [by email](mailto:d@hop.ie) or [on Twitter](http://twitter.com/donovanh). 
